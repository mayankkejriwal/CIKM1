...
//The file used for baseline experiments.


public class Phase1DedupBaseline {
	...
	public static class Map extends Mapper<LongWritable, Text, Text, Text> {
	  ...
	  //writes out non-featurized tokens as keys
	        ...
	        for(int i=1; i<tokens.length; i++){
	        	...
	        	for(String split:splits){
	        		for(String forb:Parameters.forbiddenwords)
	        			if(split.equals(forb)||split.trim().length()==0)
	        				continue;
	 ...
	 public static void main(String[] args) throws Exception {
	    Configuration conf = new Configuration();
	        ...
	        Job job = new Job(conf, "phase1dedupbaseline");
	        job.setJarByClass(Phase1DedupBaseline.class);
